...
under which MarkItUp received such components are set forth below. MarkItUp reserves all rights not expressly 
...
The following section summarizes these changes. The full details are available in the MarkItUp source code 
repository under PR #1160 (https://github.com/microsoft/markitup/pull/1160)

This project incorporates `dwml/latex_dict.py` and `dwml/omml.py` files without any additional logic modifications (which 
lives in `packages/markitup/src/markitup/converter_utils/docx/math` location). However, we have reformatted the code
