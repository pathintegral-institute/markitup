...
from markitup._uri_utils import parse_data_uri, file_uri_to_path
...
def test_docx_comments() -> None:
    ...
    markitup_with_style_map = MarkItUp(style_map="comment-reference => ")
    result = markitup_with_style_map.convert(
...
def test_input_as_strings() -> None:
    ...
    result = markitup.convert_stream(io.BytesIO(input_data))
...
def test_markitup_remote() -> None:
    markitup = MarkItUp()

    # By URL
    result = markitup.convert(PDF_TEST_URL)
    for test_string in PDF_TEST_STRINGS:
        assert test_string in result.text_content

    # Youtube
    result = markitup.convert(YOUTUBE_TEST_URL)
...
def test_speech_transcription() -> None:
    ...
    for file_name in ["test.wav", "test.mp3", "test.m4a"]:
        result = markitup.convert(os.path.join(TEST_FILES_DIR, file_name))
...
def test_exceptions() -> None:
    ...
    with pytest.raises(UnsupportedFormatException):
        markitup.convert(os.path.join(TEST_FILES_DIR, "random.bin"))
...
def test_markitup_exiftool() -> None:
    which_exiftool = shutil.which("exiftool")
    assert which_exiftool is not None

    # Test explicitly setting the location of exiftool
    markitup = MarkItUp(exiftool_path=which_exiftool)
    result = markitup.convert(os.path.join(TEST_FILES_DIR, "test.jpg"))
    ...
    result = markitup.convert(os.path.join(TEST_FILES_DIR, "test.mp3"))
...
def test_markitup_llm() -> None:
    client = openai.OpenAI()
    markitup = MarkItUp(llm_client=client, llm_model="gpt-4o")

    result = markitup.convert(os.path.join(TEST_FILES_DIR, "test_llm.jpg"))
    ...
    result = markitup.convert(os.path.join(TEST_FILES_DIR, "test.pptx"))
...
if __name__ == "__main__":
    ...
    for test in [
        ...
        test_markitup_remote,
        test_speech_transcription,
        test_exceptions,
        test_markitup_exiftool,
        test_markitup_llm,
    ]:
